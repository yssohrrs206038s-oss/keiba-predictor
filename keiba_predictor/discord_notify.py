...
def _store_prediction(race_id: str, race_name: str, race_date: str,
    ...
    try:
        ...
        _is_grade_flag = cache[race_id].get("is_grade")
        cache[race_id]["bet_strategy"] = _decide_bet_strategy(
            ...
            is_grade_override=_is_grade_flag)
    except Exception as e:
...
def _format_prediction_from_cache(race_name: str, entry: dict, race_id: str = "") -> tuple[str, str]:
    ...
    if bs and bs.get("total_points", 0) > 0:
        ...
        lines2 += [_SEP, f"合計 {display_pts}点 / {total_cost:,}円（ワイド×1,000円・3連複×100円）"]
...
def run_result_notify(
    ...
    for race in grade_races:
        ...
        # 見送り（shadow bet）レースは結果通知しない
        _bs = cache.get(race_id, {}).get("bet_strategy", {})
        if _bs and _bs.get("total_cost", 0) == 0:
            logger.info(f"  見送りレースのためスキップ: {race_name} ({race_id})")
            continue
